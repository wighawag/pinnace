...
- **mode** — per-site `ipfs` (land + pin + MFS only; the site's address is `ipfs://<cid>`, updated per deploy) or `ipns` (also publish/refresh; the site's address is `ipns://<id>`, stable).
