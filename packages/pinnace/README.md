...
### 6. Change a site's metadata (re-deploy, or `set` when you have no build)
...
When you do NOT have the build — an `ipfs`-mode site that is live, whose `./dist` is long gone — `set` changes the same metadata on the site as it stands, using the cid your nodes already hold. It is the one-command promotion to a mutable name:

```sh
# promote a live ipfs-mode site to ipns and give it an eth.limo name to warm
pinnace --config pinnace.json set --set-mode ipns --set-ens-name mysite.eth mysite
...
### Staging and promotion (the safe shape for CI)

A site wrapper MEANS "the cid this name resolves to", so a pipeline that deploys every build straight to the LIVE id breaks that meaning in `ipfs` mode, where each build has its own address and the ENS record moves only when a human moves it. The node would then warm a cid nobody resolves, report `freshness=stale` for ever, and leave the cid the record actually points at UNTRACKED (so any retention policy could reclaim the live build). Deploy to a STAGING id instead, and promote:

```sh
# CI, on every push: a real, pinned, warmed build under its own id
pinnace deploy ./web/build mandalas-staging

# you, when you have looked at it: point the live site at that exact build
pinnace pin --from-site mandalas-staging --as mandalas.eth
```

`--from-site` reads the source site's CURRENT cid from MFS on ONE node (the publisher first, falling back to the others so a down publisher cannot block a promotion) and pins THAT cid on every node, so an unevenly-landed deploy can never promote two different builds to two boxes. The content is usually already local, which makes a promotion nearly free; a node that does not hold it (a replica added later) fetches it like any other pin. The output names the node whose view was taken.

The destination keeps its OWN metadata: omitting `--set-mode` / `--set-ens-name` preserves what `mandalas.eth` stores, never what the staging site stores, so promoting a plain `ipfs` staging build into a published `ipns` site keeps signing its name. Promoting a site onto itself is refused.
...
Like `--set-mode` and `--set-ens-name`, omitting both flags PRESERVES, so a routine deploy never turns retention on, off, or up. The policy is applied as each `deploy`/`pin`/`set` writes, and can also be run on its own:
...
(`set` applies the policy too, on the same terms as `deploy`/`pin`: `pinnace set --set-keep 2 mysite` both records the policy and acts on it.)
...
Every node-touching verb (`deploy`, `set`, `pin`, `status`, `site`, `authorize`) also accepts, after the verb, `--host-endpoint.<name> <url>` / `--host-token.<name> <t>` (override one configured host).
