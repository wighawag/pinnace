...
4. As an operator, I want `deploy` to write the site's `metadata.json` (its `ensName`, `mode`) into MFS when it deploys, so that changing a site's metadata is just a re-`deploy` (idempotent), not a re-provision or a config-file edit the box never sees. (Re-running `deploy` for the same `id` IS the update, for an operator who is placing content anyway. SUPERSEDED IN PART: a later change added the metadata-only `update` verb for the case this story does not cover — a site that is already live whose build the operator no longer has, so there is no `deploy` to re-run. It writes the same `metadata.json` through the same resolver and places no content.)
