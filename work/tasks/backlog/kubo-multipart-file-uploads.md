...
Almost done — only a TypeScript build error to fix: mock-kubo.ts uses body.entries() which TS rejects (error TS2339: Property 'entries' does not exist on type 'FormData' under this lib config). FormData is directly iterable: replace 'for (const [field, value] of body.entries())' with 'for (const [field, value] of body as Iterable<[string, FormDataEntryValue]>)' OR simpler, iterate 'body.forEach((value, field) => …)'. The FormData-based fileUpload approach + the multipart mock modelling are correct — keep them; just make it compile. Then verify passes.
