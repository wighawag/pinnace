...
- **Per-site `mode` + `ensName` live in MFS metadata**, set at deploy/pin time (`--mode`, the ens-name flags), not in config. Document the three ensName verb-flags + leave-alone: `--set-ens-name <name>` (warm it), `--opt-out-ens-name` (`""`, never warm even a `.eth` id), `--unset-ens-name` (back to `.eth` inference), and OMIT = preserve (a re-deploy without an ens flag never wipes it). Note the `.eth`-id auto-inference (a site id ending `.eth` warms eth.limo automatically).
