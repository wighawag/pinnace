...
- **Per-site `mode` + `ensName` live in MFS metadata**, set at deploy/pin time (`--mode`, the ens-name flags), not in config. Document the two ensName verb-flags + leave-alone: `--set-ens-name <name>` (warm `<name>.limo`; to restore the inferred name for a `.eth` site, set it to the id), `--unset-ens-name` (opt out — never warm, even a `.eth` id), and OMIT = preserve (a re-deploy without an ens flag never changes it). Note the `.eth`-id auto-inference (a site id ending `.eth` warms eth.limo automatically unless opted out).
