...
Add `pinnace update <id>` — change a live site's metadata (mode, ensName, keep) without rebuilding or re-placing its content
...
```sh
pinnace update --set-mode ipns --set-ens-name mysite.eth mysite
```
